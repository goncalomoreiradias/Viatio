...
import { ArrowRight, Lock, Mail, User, Loader2, ArrowLeft } from "lucide-react";
            ...
            >
                <div className="glass-card p-10 sm:rounded-[2.5rem]">
                    <div className="text-center mb-8">
                        ...
                        <p className="text-gray-500 font-medium">{t("auth.register_subtitle")}</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="space-y-4">
                            <div className="relative group">
                                <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none text-gray-400 group-focus-within:text-brand-secondary transition-colors">
                                    <User size={20} />
                                </div>
                                <input
                                    type="text"
                                    ...
                                    className="w-full bg-white/50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 focus:border-brand-secondary focus:ring-4 focus:ring-brand-secondary/10 rounded-2xl pl-12 pr-5 py-4 outline-none font-medium transition-all"
                                />
                            </div>
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full py-4 bg-brand-secondary hover:bg-brand-primary text-white font-bold rounded-2xl shadow-xl hover:shadow-2xl hover:-translate-y-1 shadow-brand-secondary/30 flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 text-lg"
                        >
                            {loading ? <Loader2 className="animate-spin" size={24} /> : (
                                <>
                                    {t("auth.register_btn")} <ArrowRight size={20} className="ml-1" />
                                </>
                            )}
                        </button>
                    </form>
                    ...
                    <p className="mt-8 text-center text-gray-500 font-medium">
                        {t("auth.has_account")}{" "}
                        <Link href="/login" className="text-brand-secondary hover:text-brand-primary font-bold transition-colors">
                            {t("auth.signin_link")}
                        </Link>
                    </p>
                </div>
