...
import { ArrowRight, Lock, Mail, Loader2, ArrowLeft, Sparkles } from "lucide-react";
    ...
    return (
        <main className="min-h-screen bg-obsidian text-white relative overflow-hidden flex flex-col items-center justify-center p-6 selection:bg-accent-cobalt selection:text-white">
            ...
            <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
                <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-accent-cobalt/5 blur-[120px] rounded-full" />
                <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-accent-magenta/5 blur-[120px] rounded-full" />
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
                        ...
                        >
                            {loading ? <Loader2 className="animate-spin" size={20} /> : (
                                <>
                                    {t("auth.signin")} <ArrowRight size={16} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                                </>
                            )}
                        </button>
                    </form>
